	...
	if ($transcoder->{'command'} eq '-' && ($directUrl = $client->canDirectStream($url, $self)) && (!$redir || $client->canHTTPS)) {
		...
		if (main::TRANSCODING) {
			if ($transcoder->{'command'} ne '-' && ! $handlerWillTranscode) {
				...
				# Multi-stage transcodes (spotty|lame etc.) need IPC::Open2 (Pipeline); FileHandle->new cannot exec shell pipelines (ENOENT).
				my $needsPipeline = ($transcoder->{'command'} =~ /\|/) ? 1 : 0;
				my $usepipe = (defined $sock || (main::ISWINDOWS && $handler->isRemote) || ($needsPipeline && !main::ISWINDOWS)) ? 1 : undef;
